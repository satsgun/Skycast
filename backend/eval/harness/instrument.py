...
client chooses to expose a `last_usage` attribute; otherwise latency
alone is recorded. This keeps the committed clients untouched: the
harness instruments from outside the seam, it doesn't reach through it.
...
class InstrumentedLLMClient(LLMClient):
    ...
    async def get_structured(
        ...
        if usage is not None:
            ...
            self.total_tokens += int(usage)
        return result

    def snapshot_and_reset(self) -> tuple[float, int | None]:
        """Return (latency_ms, tokens_or_None) accumulated since last reset."""
